...
async def main():
    ...
    asyncio.create_task(auto_backup())   # ←←←←← ЭТО НОВОЕ
...
@dp.message(Command("fixtech"))
async def fix_tech_role(message: Message):
    if message.from_user.id != TECH_SPECIALIST_ID:
        await message.answer("❌ Только для Глав Тех Специалиста!")
        return

    db_user = await get_or_create_user(message.from_user.id)
    db_user.role = Role.CHIEF_TECH.value   # ← принудительно ставим правильную роль
    ...
    await message.answer(
        "✅ Роль **Глав Тех Специалист** принудительно установлена!\n\n"
        "Нажми «🔄 Обновить» — теперь все твои кнопки должны вернуться.",
        reply_markup=get_main_menu_keyboard(Role.CHIEF_TECH.value)
    )
